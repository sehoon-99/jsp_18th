...
<body>
	...
	<sql:setDataSource var="dataSource" url="jdbc:oracle:thin:@localhost:1521:xe" 
		...
		<sql:query var="rs" dataSource="${dataSource}">
			select * from newsDB where id=?
			<sql:param value="<%=id %>" />
		</sql:query>
		<c:forEach var="row" items="${rs.rows }">
		
	<div class="container-fluid">
		<form action="./processUpdateNews.jsp" method="post"
			enctype="multipart/form-data" name="newReport">
			<div class="row">
				<div class="offset-xl-2 col-4 mr-auto my-3">
					TV 홈 > 뉴스 > <span class="font-weight-bold">수정</span>
				</div>
				<div class="col-xl-8 offset-xl-2 p-0 mb-3">
					<p class="title text-white">뉴스 수정</p>
					<img
					...
					<div class="row">
						<div class="col-12 mt-3 mb-2">
							<input type="text" id="title" name="title"
								class="bg-light w-100 form-control" placeholder="제목을 입력해주세요." value="${row.title }"required>
								<input type="hidden" id="id" name="id"
								class="bg-light w-100 form-control"  value="${row.id }"required>
						</div>
						<div class="col-12 mb-3 mt-2 ">
							<textarea class="form-control bg-light"
								aria-label="With textarea" id="text" placeholder="내용을 입력해주세요."  name="text"
								rows="5" required>${row.text }</textarea>
						</div>
						...
						<div class="col-12 mt-4 mb-2 input-group">
							...
							<c:if test="${row.category ne 'naver.com' }">
								<option value="정치">정치</option>
							</c:if>
							<c:if test="${row.category ne 'gmail.com' }">
								<option value="사회">사회</option>
							</c:if>
							<c:if test="${row.category ne 'nate.com' }">
								<option value="경제">경제</option>
							</c:if>
							<c:if test="${row.category ne 'daum.net' }">
...
<div class="line"></div>
			<div class="row py-5">
				...
				<div class="col-12 text-center">
					 <input type="submit" class="btn btn-primary w-25 p-3" value="수정하기">
				</div>
			</div>
		</form>
	</div>
